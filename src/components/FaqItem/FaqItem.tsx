...
import { FaqSliceDefaultItem, Simplify } from "../../../prismicio-types";
import { PrismicRichText } from "@prismicio/react";

import { useState } from "react";

interface IProps {
  item: Simplify<FaqSliceDefaultItem>;
}
...
const FaqItem: React.FC<IProps> = ({ item }) => {
  ...
  return (
    <div className={css.wrap} onClick={() => setIsOpen(!isOpen)}>
      <div className={css.heading}>
        <PrismicRichText
          field={item.question}
          components={{
            heading3: ({ children }) => (
              <h3 className={css.question}>{children}</h3>
            ),
          }}
        />
        {isOpen ? (
          <CloseEyeIcon className={css.closeIcon} />
        ) : (
          <EyeIcon className={css.openIcon} />
        )}
      </div>
      {isOpen && (
        <PrismicRichText
...
function CloseEyeIcon({ className }: { className?: string }) {
  return (
    <svg
      className={className}
      xmlns="http://www.w3.org/2000/svg"
